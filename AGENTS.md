...
## Current status
**v0.1.11 — Toast notification watcher + F10 hotkey + auto-play (2026-06-09).** New `NotificationWatcher` in `src/core/notification_watcher.py` polls `%LOCALAPPDATA%\Microsoft\Windows\Notifications\wpndatabase.db` every 500ms from a daemon thread, detects new notifications across all apps. Bridge signal `_NotificationBridge.detected(str, str)` carries human text + app_id to the UI thread. Auto-play triggers playback when notification text contains death keywords. F10 hotkey added to `HotkeyManager` for record/stop toggle. Double-click on recording list opens rename dialog. Theme redesigned to monochrome dark (`#050505` bg, `#f5f5f5` text).
...
- **`storage.py`** — `save_recording` / `load_recording`. JSON, indent=2, validates schema strictly (`_validate_payload` raises on bad input). Also `rename_recording()`.
- **`hotkey.py`** — global F9/F10/ESC listener via `pynput.keyboard.Listener`. F9/F10 callbacks use `Signal.emit()` on the UI-thread bridge to hop to the UI thread. ESC callback only sets a flag.
- **`notification_watcher.py`** — `NotificationWatcher` polls `wpndatabase.db` from a daemon thread. `get_available_apps()` returns unique AppIds. Detects new notifications across all apps, calls `callback(payload, text, app_id)`.

### `src/ui/` — Qt layer
- **`app.py`** — `QMainWindow` with `FramelessWindowHint` + `WA_TranslucentBackground`. Custom title bar (drag region + min/close). Mica/Acrylic applied via `DwmSetWindowAttribute` (`DWMWA_SYSTEMBACKDROP_TYPE` = 2 for Mica, 3 for Acrylic fallback). **Contains `_PlayerBridge(QObject)`** — a tiny QObject with Signals (`progress(int, int)`, `done()`, `hotkey_play()`, `hotkey_record()`, `hotkey_cancel()`) used for thread-safe worker → UI communication. Also **`_NotificationBridge(QObject)`** with `detected(str, str)` for notification watcher → UI. Both are constructed on the UI thread, so their Signal emissions automatically queue slots back onto the UI thread.
